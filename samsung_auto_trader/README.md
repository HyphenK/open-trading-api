...
아래 내용 중 수량에 관한 부분은, 코드 작성 당시 삼성전자 주가가 20만원 초반대였기에 이를 바탕으로 작성한 부분이며, 현재 삼성전자 주가는 30만원 초반대이므로 첨부한 6월 1일자 로그에서는 "즉 **최대 보유 상태에서도 약 20% 현금 버퍼**가 남도록 잡은 설정입니다." 라는 아래의 설명에 부합하도록 올바른 값으로 수정한 후 프로그램을 작동하였습니다.
